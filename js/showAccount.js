      ...
      $.getJSON(url,function(data){
        ...
        for(var i=0;i<500;i++){
            if($('#userRole').val()=="Admin") {
                $('#Transactions').append("<tr><td>" + rows[i]['id'] + "</td><td>" + rows[i]['date'] + "</td><td>" + rows[i]['dis'] + "</td><td>Rs " + parseFloat(rows[i]['debit']).toLocaleString() + "</td><td>Rs " + parseFloat(rows[i]['credit']).toLocaleString() + "</td><td ><a class='btn btn-sm btn-primary' onclick='editArow(this);'><i class='fa fa-edit'></i>&nbsp;Edit</a></td><td><a class='btn btn-sm btn-danger glyphicon glyphicon-remove row-remove'  onclick='deletetr(this);'></a></td></tr>");
            }
            else
                {

                    $('#Transactions').append("<tr><td>" + rows[i]['id'] + "</td><td>" + rows[i]['date'] + "</td><td>" + rows[i]['dis'] + "</td><td>Rs " + parseFloat(rows[i]['debit']).toLocaleString() + "</td><td>Rs " + parseFloat(rows[i]['credit']).toLocaleString() + "</td><td ><a class='btn btn-sm btn-primary' style='display: none';  onclick='editArow(this);'><i class='fa fa-edit'></i>&nbsp;Edit</a></td><td><a class='btn btn-sm btn-danger glyphicon glyphicon-remove row-remove'" +
        ...
        if(fdate!="00/00/0000"&&tdate=="00/00/0000"){
            ...
            for(var i=0;i<rows.length;i++){
                if(toDate(fdate).getTime()===toDate(rows[i]['date']).getTime()){
                    flag=true;
                    if($('#userRole').val()=="Admin") {
                        $('#Transactions').append("<tr class='success'><td>" + rows[i]['id'] + "</td><td>" + rows[i]['date'] + "</td><td>" + rows[i]['dis'] + "</td><td>Rs " + parseFloat(rows[i]['debit']).toLocaleString() + "</td><td>Rs " + parseFloat(rows[i]['credit']).toLocaleString() + "</td><td ><a class='btn btn-sm btn-primary' onclick='editArow(this);'><i class='fa fa-edit'></i>&nbsp;Edit</a></td><td><a class='btn btn-sm btn-danger glyphicon glyphicon-remove row-remove'  onclick='deletetr(this);'></a></td></tr>");
                    }
                    else
                    {

                        $('#Transactions').append("<tr class='success'><td>" + rows[i]['id'] + "</td><td>" + rows[i]['date'] + "</td><td>" + rows[i]['dis'] + "</td><td>Rs " + parseFloat(rows[i]['debit']).toLocaleString() + "</td><td>Rs " + parseFloat(rows[i]['credit']).toLocaleString() + "</td><td ><a class='btn btn-sm btn-primary' style='display: none';  onclick='editArow(this);'><i class='fa fa-edit'></i>&nbsp;Edit</a></td><td><a class='btn btn-sm btn-danger glyphicon glyphicon-remove row-remove'" +
                            " style='display: none';  onclick='deletetr(this);'></a></td></tr>");
                    }
                }
...
function saveTransaction()
{
 ...
 $.ajax({
                                ...
                                data:{aid:$("#aid").val(),tdate:$("#trdate").val(),dis:$('#trdis').val(),debit:$('#trdebit').val(),credit:$('#trcredit').val()},
                                success: function(msg){
...
function (isConfirm) {
    if (isConfirm) {
        $.ajax({
            ...
            type: "POST",
            data: { id:$('#tid').html(),dis:$('#tdis').val(),recieved:$('#treceived').val(),paid:$('#tpaid').val()},
            success: function (msg) {
