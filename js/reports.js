...
function showStockReport() {
    if (!$('#stock_report_type').val() || !$('#stock_report_from').val() || !$('#stock_report_to').val() || toDate($('#stock_report_from').val()).getTime() > toDate($('#stock_report_to').val()).getTime()) {
        swal("Input Error!", "Please fill all required fields", "error");
        return;
    }
    setTimeout("location.href='views/stockReport.php?report_type=" + $('#stock_report_type').val() + "&from=" + $('#stock_report_from').val() + "&to=" + $('#stock_report_to').val() + "'", 1000);
}