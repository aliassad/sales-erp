...
$(document).ready(function () {
    ...
    $.getJSON(url, function (data) {
        ...
        $.each(data, function (index, data) {
            $('#product1').append('<option value="' + data.id + '">' + data.article_no + ' ' + data.origin + ' ' + data.des + ' ' + data.item_length + '*' + data.item_width + ': ' + ((data.item_length * data.item_width) / 10000).toFixed(2) + ' SQM </option>'
            )
            ;
        });
        $('#product1').addClass('selectpicker');
    }).then(function () {
        $('.selectpicker').selectpicker('render');
    });
});
...
function calAmount(row) {
    ...
    var selected_product = products().filter([{id: {"===": prod}}]).get();
