...
function clearModal() {
    ...
    var url = 'views/gettingTeppichBillPayments.php?id=' + $('#billno').val();
    ...
    swal({
        ...
        function (isConfirm) {
            if (isConfirm) {
                ...
                $.ajax({
                    type: "POST",
                    url: "views/deleteTeppichBill.php",
                    data: {
                        bid: id
                    },
                ...
                setTimeout('location.href="index.php?page=teppich_sell"', 2000);
            } else {
                swal({
                    title: "Cancelled",
                    type: "error",
...
function payAmount() {
    ...
    $.ajax({
        type: "POST",
        url: "views/payTeppichBillAmount.php",
        data: {
            bid: $('#billno').val(),
            balance: rb,
        ...
        success: function (msg) {
            ...
            setTimeout('location.href="index.php?page=showTeppichBill&billno=' + $('#billno').val() + '"', 1500);
        }
    });
...
function deleteArow(id) {
    ...
    swal({
        ...
        function (isConfirm) {
            if (isConfirm) {
                $.ajax({
                    url: 'views/deleteTeppichBillPayments.php',
                    type: "POST",
                    data: {rid: rowid},
                    success: function (msg) {
