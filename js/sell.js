...
$(document).ready(function(){
     ...
     var url = 'views/gettingBills.php';
      $.getJSON(url,function(data){
        ...
        for(var i=0;i<500;i++)
          $('#bills').append("<tr onclick='showbill(this);'><td>"+rows[i]['id']+"</td><td>"+rows[i]['name']+"</td><td>Rs "+parseFloat(rows[i]['amount']).toLocaleString()+"</td><td>Rs "+parseFloat(rows[i]['paid']).toLocaleString()+"</td><td>Rs "+parseFloat(rows[i]['balance']).toLocaleString()+"</td><td>"+rows[i]['date']+"</td><td>"+rows[i]['ddate']+"</td><td><span class='btn btn-xs btn-info'>"+rows[i]['type']+"</span></td></tr>"); 
            ...
            if(sid||sname!="999"||fdate!="00/00/0000"&&tdate=="00/00/0000"){
                ...
                for(var i=0;i<rows.length;i++){
                    if(sname==rows[i]['name']||sid==rows[i]['id']||toDate(fdate).getTime()===toDate(rows[i]['date']).getTime()){
                        flag=true;
                        $('#bills').append("<tr onclick='showbill(this);' class='success'><td>"+rows[i]['id']+"</td><td>"+rows[i]['name']+"</td><td>Rs "+parseFloat(rows[i]['amount']).toLocaleString()+"</td><td>Rs "+parseFloat(rows[i]['paid']).toLocaleString()+"</td><td>Rs "+parseFloat(rows[i]['balance']).toLocaleString()+"</td><td>"+rows[i]['date']+"</td><td>"+rows[i]['ddate']+"</td><td><span class='btn btn-xs btn-info'>"+rows[i]['type']+"</span></td></tr>");
                    }
