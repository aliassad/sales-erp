...
function loadProductDetail() {
    ...
    var prods = products({id: $('#vproduct').val()}).get();
    $('#vrate').val(prods[0]['purchaseprice']);
    $('#vqty').val(1);
    $('#vqty').focus();
}
...
function saveVendorVoucher() {
    ...
    $('#vendorvoucher tr').each(function (row, tr) {
        TableData[row] = {
            "product": $(tr).find('td:eq(1)').attr('id')
            , "rate": $(tr).find('td:eq(2)').html()
            , "qty": $(tr).find('td:eq(3)').html()
            , "amount": $(tr).find('td:eq(4)').html()
    ...
    $.ajax({
        ...
        data: {
            vid: $("#vid").val(),
            pTableData: fTableData,
            payment: $("#vpayment").val(),
            bno: $('#bno').val(),
            vdate: $('#vdate').val(),
            ptype: $("#ptype").val(),
            pdetail: $("#pdetail").val(),
...
function getVendorData() {
    ...
    $.getJSON(url, function (data) {
        $.each(data, function (index, data) {
            ...
            products = TAFFY(data.products);
            $('#vproduct').addClass('selectpicker');
            $.each(data.products, function (index, data) {
                $('#vproduct').append('<option value="' + data.id + '">' + data.article_no + ' ' + data.origin + ' ' + data.des + ' ' + data.item_length + '*' + data.item_width + ': ' + ((data.item_length * data.item_width) / 10000).toFixed(2) + ' SQM </option>');
            });
        });
    }).then(function () {
...
function saveVo() {
    ...
    var selected_product = products({id: $('#vproduct').val()}).get();
    var sqm= (selected_product[0]['item_length']*selected_product[0]['item_width']/10000).toFixed(2);

    $('#vendorvoucher').append("<tr><td class='itemno'></td><td id='" + $('#vproduct').val() + "'>" + $('#vproduct option:selected').text() + "</td><td>" + $('#vrate').val() + "</td><td>" + $('#vqty').val() + "</td><td class='amt'>" + parseFloat($('#vqty').val()) * parseFloat($('#vrate').val()*sqm).toFixed(2) + "</td><td><a style='padding: 2px 6px;' class='btn btn-danger glyphicon glyphicon-remove row-remove'  onclick='deletetrV(this);'></a></td></tr>");
