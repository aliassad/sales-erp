...
$(document).ready(function(){
     ...
     var url = 'views/gettingVendors.php';
      $.getJSON(url,function(data){
          ...
          var total_payable=0;
          if (data == "") {
              $('#vendors').append("<tr class='danger'><td colspan='8'><center><b>No Vendors Found !...</b></center></td></tr>");
              return;
          }
          $.each(data,function(index,data) {
              total_payable+=parseFloat(data.payment);
                    $('#vendors').append('<tr onclick="showEmployee(this);" ><td>'+data.id+'</td><td>'+data.name+'</td><td>'+data.phone+'</td><td>'+data.city+'</td><td>'+data.address+'</td><td>'+data.email+'</td><td>'+data.cname+'</td><td>Rs '+parseFloat(data.payment).toLocaleString()+'</td></tr>');
    });

          $('#total_Payable').val('Rs '+parseFloat(total_payable).toLocaleString());
...
function filter(){
      ...
      rows=db().filter([{id:pid},{name:{likenocase:pname}},{city:{likenocase:pcity}}]).get();
       if(rows.length==0)
       {
           total_payable=0;
           $('#vendors').append("<tr class='danger'><td colspan='8'><center><b>No result found..!</b></center></td></tr>");
       }
      else
       {
           total_payable=0;
         for(var i=0;i<rows.length;i++){
             ...
             $('#vendors').append('<tr class="success" onclick="showEmployee(this);" ><td>'+rows[i]['id']+'</td><td>'+rows[i]['name']+'</td><td>'+rows[i]['phone']+'</td><td>'+rows[i]['city']+'</td><td>'+rows[i]['address']+'</td><td>'+rows[i]['email']+'</td><td>'+rows[i]['cname']+'</td><td>Rs '+parseFloat(rows[i]['payment']).toLocaleString()+'</td></tr>');
         }
       }
    ...
    else
    {
        ...
        rows=db().get();
           for(var i=0;i<rows.length;i++) {
               total_payable+=parseFloat(rows[i]['payment']);
               $('#vendors').append('<tr onclick="showEmployee(this);" ><td>'+rows[i]['id']+'</td><td>'+rows[i]['name']+'</td><td>'+rows[i]['phone']+'</td><td>'+rows[i]['city']+'</td><td>'+rows[i]['address']+'</td><td>'+rows[i]['email']+'</td><td>'+rows[i]['cname']+'</td><td>Rs '+parseFloat(rows[i]['payment']).toLocaleString()+'</td></tr>')
           }
    }
     ...
     $.ajax({
                  ...
                  success:function (msg) {
                   ...
                   if (msg == 'true') {
                    ...
                    setTimeout(function(){"location.href='index.php?page=vendors'"},2000);
