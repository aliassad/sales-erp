...
$r=query("select id from customer where concat(id,': ',name)='$cname'");
 ...
 $r=query("UPDATE  bill SET `cid`='$id',`discount`='$discount',`amount`='$gtotal',date=STR_TO_DATE('$date', '%d-%m-%Y'),ddate=STR_TO_DATE('$duedate', '%d-%m-%Y'),notes='$notes',type='$type' where id='$bid'");
 if(!$r)
 {
     echo "false1";
 }   
...
//                    {
//                    echo $s."   ".$bid."    ".$rate."   ".$unit."   ".$disc."   ".$amount;
//
//                     break;
//                    }
            }
