...
$result=query("select sum(amount) as credit  from `vendorpayments` where vid='$pid' and date < STR_TO_DATE('$from','%d-%M-%Y') and ptype='Credit';");
while($row=mysqli_fetch_array($result))
    $totalCredit+=$row['credit'];


$result=query("select sum(amount) as debit  from `vendorpayments` where vid='$pid' and date < STR_TO_DATE('$from','%d-%M-%Y') and ptype='Debit';");
while($row=mysqli_fetch_array($result))
...
$result=query("select id,concat('Payment:',' ',pdetail) as des,amount,ptype,DATE_FORMAT(date,'%d-%M-%Y') as date from `vendorpayments` where 
vid='$pid' and date BETWEEN 
STR_TO_DATE
('$from','%d-%M-%Y') AND STR_TO_DATE('$to','%d-%M-%Y') ");
