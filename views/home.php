<?php if (isset($_GET['page'])) $page = $_GET['page']; else {
    $page = "home";
}
...
-->
    ...
    <?php //echo number_format($cheques); ?><!--</span>-->
    <!--                    </a>-->
    ...
    <div class="row" style="padding:10px;">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title"><i class="fa fa-dashboard"></i><span class="break"></span>&nbsp;Dashboard panel
                </h3>
            </div>
                ...
                <div class="row">
                    <div class="col-lg-4">
                        <div class="panel panel-blue">
                            <div class="panel-heading">
                                <h3 class="panel-title"><i class="fa fa-group"></i><span class="break"></span>&nbsp;Members
                                    activities</h3>
                            </div>
                            <div class="panel-body panel-notify" style="height:300px;">

                                <?php $result = query("select m.id,m.name,m.cardno,m.name,sum(me.amount) amount from member m, mentry me where m.cardno=me.mno group by m.id order by sum(me.amount) desc  ;");
                                while ($row = mysqli_fetch_array($result)) {
                                    echo '<ul class="notification-panel" ><li><img class="dashboard-avatar" src="img/img.jpg"><b>Name:</b><a href="index.php?page=showMember&idno=' . $row['id'] . '">' . $row['name'] . '.</a></li><li><b>Member Ship No:</b>&nbsp;' . $row['cardno'] . '.</li><li><b>Total Sale Amount:</b>&nbsp;' . number_format($row['amount'],',','.').CURRENCY_SIGN . '.</span></li></ul>';
                                } ?>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <div class="panel panel-blue">
                            <div class="panel-heading">
                                <h3 class="panel-title"><i class="fa fa-money"></i><span class="break"></span>&nbsp;Payments
                                    pending</h3>
                            </div>
                            <div class="panel-body panel-notify" style="height:300px;">
                                ...
                                while ($row = mysqli_fetch_array($result)) {
                                    ...
                                    if ($row['days'] == -1) {
                                        ...
                                        $color = "info";
                                        echo '<ul class="notification-panel" ><li><i class="dashboard-avatar fa fa-money fa-4x" style="border:none;"></i><b>Bill no:&nbsp;</b><a href="index.php?page=showbill&billno=' . $row['id'] . '">' . $row['id'] . '.</a></li><li class="label label-md label-' . $color . '" >' . $dis . '</li><li><b> Payment:</b>&nbsp;' . number_format($pending, 2,',','.') .CURRENCY_SIGN. '.</li></ul>';
                                    } else
                    ...
                    <div class="col-lg-4">
                        <div class="panel panel-blue">
                            <div class="panel-heading">
                                <h3 class="panel-title"><i class="fa fa-briefcase"></i><span class="break"></span>&nbsp;Inventory
                                    Notifications</h3>
                            </div>
                            <div class="panel-body panel-notify" style="height:300px;">

                                <?php $result = query("select p.id,p.des,p.stock from product p where p.stock<=p.minstock  order by p.stock;");
                                while ($row = mysqli_fetch_array($result)) {
                                    echo '<ul class="notification-panel" ><li><img class="dashboard-avatar" src="img/products.png"><b>Product No#&nbsp;</b>' . $row['id'] . '.</li><li><b>Product des:&nbsp;</b>' . $row['des'] . '.</li><li><b>Current Stock:&nbsp;</b>&nbsp;' . $row['stock'] . '.</li></ul>';
                                } ?>
                            </div>
                        </div>
                    </div>
...
<!-- /#page-wrapper -->
<!-- jQuery -->
<div class="modal fade" id="paymentReceivingVoucher" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
                    ...
                    <div class="row">
                        <div class="col-md-6" style="padding-left:0px; padding-bottom:3px;">
                            <div class="input-group">
                                ...
                                <input id="serialNo" type="text" class="form-control" readonly tabindex="-1"
                                       value="PR-XXXX"/>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="input-group">
                                ...
                                <input id="paymentReceivingDate" name="paymentReceivingDate" class="form-control"
                                       type="text"/>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6" style="padding-left:0px; padding-bottom:3px;">
                            <div class="input-group">
                                ...
                                <select id="customerId" name="customerId" class="selectpicker show-tick"
                                        data-live-search="true" title="Customer"
                        ...
                        <div class="col-md-6" style="padding-right:0px">
                            <div class="input-group">
                                <span class="input-group-addon"><i
                                            class="fa fa-credit-card"></i>&nbsp;Payment Type</span>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" onclick="submitPaymentReceivingVoucher();"><i
                            class="fa fa-save"></i>&nbsp;Save changes
                </button>
                <button type="button" class="btn btn-primary" onclick="submitAndPrintPaymentReceivingVoucher();"><i
                            class="fa fa-print"></i>&nbsp;Save &amp; Print
                </button>
                <button type="button" id="btn_close" class="btn btn-default" data-dismiss="modal"><i
...
<div class="modal fade" id="paymentPayingVoucher" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header" style="padding: 5px;">
                ...
                <form id="paymentPayingForm">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="input-group">
                                <span class="input-group-addon"><i class="fa  fa-calendar"></i>&nbsp;Date</span>
                                <input id="paymentPayingDate" name="paymentPayingDate" class="form-control" type="text"
                                       required/>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6" style="padding-left:0px; padding-bottom:3px;">
                            <div class="input-group">
                                ...
                                <select id="vendorId" name="vendorId" class="selectpicker show-tick"
                                        data-live-search="true" title="Select Vendor"
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" onclick="submitPaymentPayingVoucher();"><i
                            class="fa fa-save"></i>&nbsp;Save
                </button>
                <button type="button" class="btn btn-primary" onclick="submitAndPrintPaymentPayingVoucher();"><i
                            class="fa fa-print"></i>&nbsp;Save &amp; Print
                </button>
                <button type="button" id="btn_close" class="btn btn-default" data-dismiss="modal"><i
                            class="fa fa-remove"></i>&nbsp;Close
